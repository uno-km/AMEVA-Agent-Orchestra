...
PROMPTS = {
    ...
    "pm": (
        ...
        "9. [보안 최우선 원칙]: 코드가 다소 길어지고 구현이 복잡해지더라도, 시스템 안정성과 보안이 최우선입니다. 개발자에게 지시할 때 eval(), exec(), subprocess, os.system 등 위험한 동적 실행/OS 제어 명령을 절대 사용하지 말고, 안전한 라이브러리나 수식 파서를 자체 구현하도록 강력하게 통제하십시오.\n\n"
    ...
    "architect": (
        ...
        "6. [보안 설계 및 감시]: 코드를 더 많이 작성하더라도 보안과 안전성이 최우선입니다. 설계 시 보안에 위험한 함수(eval, exec, subprocess 등)를 배제하는 아키텍처를 잡으십시오. 코드 리뷰 시 eval(), exec() 등을 사용하는 흔적이 보이면 가차없이 반려(rejected)하고 대안 로직 작성을 지시하십시오.\n\n"
    ...
    "dev": (
        ...
        "4. [보안 최우선 - 코드가 많아져도 안전하게]: 코드를 더 많이 작성하더라도 보안이 가장 중요합니다. eval(), exec(), os.system(), subprocess 등은 강력한 보안 스캐너에 의해 즉시 차단(밴)되며 시스템이 멈춥니다! 계산기 등을 만들 때도 코드가 많이 늘어나더라도 절대 eval()을 쓰지 말고 수식 파서를 안전하게 직접 구현하십시오.\n"
    ...
    "tester": (
        ...
        "5. [보안 검증]: 개발된 코드에 보안 취약점이 없는지, eval()이나 exec()처럼 입력을 무검증으로 실행하는 위험한 구조가 포함되어 있지 않은지 테스트 케이스를 통해 엄격하게 검증하십시오.\n\n"
