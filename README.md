...
빈 폴더에서 **PowerShell(관리자 권한)**을 열고 아래 명령어를 복사하여 붙여넣으십시오. 저장소 클론부터 의존성 설치, GPU 가속 빌드 및 시스템 가동까지 단 한 번에 진행됩니다.
```gitbash
https://github.com/uno-km/AMEVA-Agent-Orchestra.git && cd AMEVA-Agent-Orchestra && ./launch.bat
```
```powershell
Set-ExecutionPolicy -ExecutionPolicy Bypass -Scope Process -Force; git clone https://github.com/uno-km/AMEVA-Agent-Orchestra.git; cd AMEVA-Agent-Orchestra; .\launch.bat
```
---
